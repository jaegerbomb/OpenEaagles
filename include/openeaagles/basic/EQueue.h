//------------------------------------------------------------------------------
// Template EQueue<T>
//      and Eaagles::Basic::Object::EQueue<T>
//
...
//
// Examples:
//    EQueue<int>* q1 = new EQueue<int>(100); // queue size 100 items
//    q1->put(1);           // puts 1 on the queue
//    q1->put(2);           // puts 2 on the queue
//    int i = q1->get();    // i is equal to 1
//    int j = q1->get();    // j is equal to 2
//------------------------------------------------------------------------------
template <class T> class EQueue {
public:
   EQueue(const unsigned int qsize) : SIZE(qsize), in(0), n(0), semaphore(0)   { queue = new T[SIZE]; }
   EQueue(const EQueue<T> &q1) : SIZE(q1.SIZE), in(0), n(0), semaphore(0)      { queue = new T[SIZE]; }
   ~EQueue()                                                                   { delete[] queue; }
...
private:
   EQueue<T>& operator=(EQueue<T>&) { return *this; }
   T* queue;                // The Queue
